...
export function Weekchart(data) {
    ...
    const APICALLInst1 = '/insights?metric=impressions,reach&period=week'
    const [weekAnalyticsInts, setWeekAnalytics] = useState([]);
            ...
            window.FB.api(
                ...
                function (response) {
                    if (response && response.data) {
                        ...
                        setWeekAnalytics(formatWeek(getAnalyticsInst(response)))
                        setLoading(false);
                    } else {
                        setError(true);
                        setLoading(false);
    ...
    const state = {
        ...
        options: {
            chart: { id: "basic-bar" },
            xaxis: { categories: weekAnalyticsInts[0] || [0] },
            colors: ['#87EB8C'],
        },
        series: [
            {
                name: "series-1",
                data: weekAnalyticsInts[1] || [0]
            }
        ]
    };
