...
export function Monthchart(data) {
    ...
    const APICALLInst1 = '/insights?metric=impressions,reach&period=days_28'
    const [monthAnalyticsInts, setMonthAnalytics] = useState([]);
            ...
            window.FB.api(
                ...
                function (response) {
                    if (response && response.data) {
                        ...
                        setMonthAnalytics(formatMonth(getAnalyticsInst(response)))
                        setLoading(false);
                    } else {
                        setError(true);
                        setLoading(false);
    ...
    const state = {
        ...
        options: {
            chart: { id: "basic-bar" },
            xaxis: { categories: monthAnalyticsInts[0] || [0] },
            colors: ['#AA5522'],
        },
        series: [
            {
                name: "series-1",
                data: monthAnalyticsInts[1] || [0]
            }
        ]
    };
